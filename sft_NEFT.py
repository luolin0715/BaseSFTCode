...
class SFTDataset(Dataset):
    def __init__(self, file_path, tokenizer):
        ...
        self.examples = self.load_data(file_path)
    ...
    def __getitem__(self, index):
        ...
        chat = self.tokenizer.apply_chat_template(dialogs, tokenize = False)
        return chat
...
model = AutoModelForCausalLM(model_path, quantization_config = bnb_config)
...
sft_dataset = SFTDataset("", tokenizer)
...
for i in range(epoch):
    for inputs, loss_mask in data_loader:
        ...
        input_embeddings = input_embeddings + torch.zeros_list(input_embeddings).uniform_(-mag_norm, mag_norm)
